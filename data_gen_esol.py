...
def scaffold_split(df_esol_prop, frac_train=0.8, frac_valid=0.1, frac_test=0.1):
    rng = np.random.RandomState(43)
    scaffolds = defaultdict(list)
    for _, row in df_esol_prop.iterrows():
        scaffold = generate_scaffold(row.smiles)
        scaffolds[scaffold].append(row['Compound ID'])

    scaffold_sets = rng.permutation(list(scaffolds.values()))
    n_total_valid = int(np.floor(frac_valid * df_esol_prop.shape[0]))
    n_total_test = int(np.floor(frac_test * df_esol_prop.shape[0]))
...
def get_and_save_data_batch(esol_df, data_idx, dest_data_path):
    rs = RunningStats(1)
    batch = 0
    for mol_id in data_idx:
        row = esol_df[esol_df['Compound ID'] == mol_id].iloc[0]
...
if __name__ == "__main__":
    esol_df = pd.read_csv("./esol_prop.csv")
    train_idx, val_idx, test_idx = scaffold_split(esol_df)
    breakpoint()
    target_name = [
        "measured log solubility in mols per litre",
    ]
    train_path = '/mnt/transvae_esol_prop_scaffold/train_data/train_batch/'
    val_path = '/mnt/transvae_esol_prop_scaffold/test_data/val_batch/'
    test_path = '/mnt/transvae_esol_prop_scaffold/test_data/test_batch/'

    create_folder(train_path)
    create_folder(val_path)
    create_folder(test_path)
    get_and_save_data_batch(esol_df, train_idx, train_path)
    get_and_save_data_batch(esol_df, val_idx, val_path)
    get_and_save_data_batch(esol_df, test_idx, test_path)
