...
from .CONSTS import (MAX_NUM_ATOMS, FEATURE_DEPTH, OUTPUT_DEPTH, HIDDEN_SIZE)
...
def get_g_core_model():
    ...
    out = conv2d_block(p6, 512)
    out = tf.keras.layers.GlobalMaxPooling2D()(out)
    out = tf.keras.layers.LayerNormalization()(out)
    out = tf.keras.layers.Activation("relu")(out)
    hg = tf.keras.layers.Dense(HIDDEN_SIZE, use_bias=False)(out)
    g_net = Model(inputs, hg, name="distance_net")
    return g_net
...
def get_gr_core_model():
    ...
    out = tf.keras.layers.LayerNormalization()(out)
    out = tf.keras.layers.Activation("relu")(out)
    hg = tf.keras.layers.Dense(HIDDEN_SIZE * 2, use_bias=False)(out)
    gr_net = Model(inputs, hg, name="distance_net")
    return gr_net
...
def get_decode_core_model():
    ...
    R = tf.keras.layers.LayerNormalization()(R)
    R = tf.keras.layers.Activation("relu")(R)
    R = tf.keras.layers.Dense(MAX_NUM_ATOMS * 3, use_bias=False)(R)
    return R
